...
### 🎛️ 共通
* **翻訳およびスペルチェック対応言語の追加**: 設定から希望の言語を追加して使用できるようになりました。
* **コードブロックのコピーボタン**: 右上に固定されていなかった問題を修正しました。
* **メインツールバーのテーマボタン**: 初めてのユーザー向けに、テーマボタンは設定で有効にできるオプションになりました。

### 🖥️ macOS
* **編集モードのバグ修正**: 変更を適用して終了する際にアプリがフリーズする現象を修正しました。

### 🍎 macOS, iPadOS/iOS
* **Apple Intelligenceの導入**: 対応環境において、AIプロバイダーとしてApple Intelligenceを選択できるようになりました。この場合、別途LLMサーバーを用意する必要はありません。ただし、コンテキスト長の制限により動作が制限される場合があります。

### 📱 Android, iPadOS/iOS
* **Ask AI呼び出しの改善**: タブレットおよびモバイルでタップ選択されたテキストに対してAsk AIプロンプトウィンドウが呼び出されなかった問題を修正しました。
...
* **AIツールバーが残ってしまう問題**: エディターのタブではない状態でAIツールバーが消えない問題を解決します。
...
* **非侵襲的なOSファイル関連付け**: 非侵襲的文件関連付け機能により、.txt、.py、.c、.mなど様々なプレーンテキストおよびソースコードファイルを開くことができます。これは最小限の介入によるファイル関連付け方式です。
* **コードおよびスクリプト編集サポート**: 様々なコードやスクリプトソースを構文ハイライト機能を使用して編集・閲覧できます。


### 📱 Android, iPadOS/iOS
* **フローティングナビゲーターのサイズ調整**: 検索やスペルチェックに使用されるフローティングボタンのタッチ領域を大きくし、タッチスクリーンでのユーザビリティを向上させました。


### 📱 iPadOS
* **スクロールバーのバグ修正**: サイドパネルが開いているときにスクロールバーが常に表示されるべきでしたが、消えてしまう現象がありました。
